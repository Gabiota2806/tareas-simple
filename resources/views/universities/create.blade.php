    ...
    <div class="py-6">
        <div class="max-w-sm mx-auto sm:max-w-md">
            <div class="bg-white shadow-md rounded-xl p-4 sm:p-6">
                ...
                @if(session('warning'))
                    <div class="mb-4 bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-lg text-sm font-bold text-center">
                        {{ session('warning') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('universities.store') }}" class="space-y-4">
                    ...
                    <div class="pt-4 flex gap-3">
                        @if(\App\Models\University::where('user_id', Auth::id())->count() > 0)
                            ...
                            <button type="submit"
                                    class="w-2/3 rounded-lg bg-violet-600 px-4 py-2 font-semibold text-white shadow-sm transition hover:bg-violet-700 hover:-translate-y-0.5 hover:shadow-md font-nunito text-sm">
                                Guardar
                            </button>
                        @else
                            <button type="submit"
                                    class="w-full rounded-lg bg-violet-600 px-4 py-2 font-semibold text-white shadow-sm transition hover:bg-violet-700 hover:-translate-y-0.5 hover:shadow-md font-nunito text-sm">
                                Comenzar
                            </button>
                        @endif
                    </div>
